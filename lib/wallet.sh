...
# Fetch /wallet/{pk}/balance. Stdout: response body. Globals set:
#   WALLET_HTTP_CODE — last HTTP status (string)
# Returns 0 always (caller checks WALLET_HTTP_CODE).
wallet_get_balance() {
    local pk="$1"
    local resp
...
# Fetch /cryptarchia/info and parse the tip HeaderId (hex). Empty on failure.
wallet_get_tip() {
    local body
...
# Stdout: response body. Globals set:
#   WALLET_HTTP_CODE — HTTP status string
...
# Stdout: response body. Globals set: WALLET_HTTP_CODE.
wallet_get_tx() {
    local hash="$1"
    local resp
    resp="$(curl -s -m "$LOGOS_API_TIMEOUT" -w '\n%{http_code}' \
...
# success, empty on failure. Caller falls back to error display.
...
# `cap` chars. Used for inline error display.
wallet_squash_body() {
    local body="$1"
    local cap="${2:-120}"
    local out
